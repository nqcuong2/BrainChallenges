...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler {
	...
	private float decelerateTime = 0f;

	public float DecelerateTime {
		set {
			decelerateTime = value;
		}
	}
	...
	public void ChangeTut() {
		...
		if (UIController.Instance.GMode != GameController.instance.GameModel.GameMode) {
			if (GameController.instance.GameModel.GameMode == GameModel.Gamemode.COLOR || GameController.instance.GameModel.GameMode == GameModel.Gamemode.SYMMETRY) {
				InitPageSelection ();
			}

			if ((UIController.Instance.GMode == GameModel.Gamemode.COLOR || UIController.Instance.GMode == GameModel.Gamemode.SYMMETRY) &&
			    (GameController.instance.GameModel.GameMode != GameModel.Gamemode.COLOR || GameController.instance.GameModel.GameMode != GameModel.Gamemode.SYMMETRY)) {
				InitPageSelection ();
			}
		}
    ...
    void Update() {
        ...
        if (_lerp) {
			...
			if (PlayerPrefs.GetInt ("Slide", 0) == 0) {
				AudioController.instance.AdSource.PlayOneShot (AudioController.instance.audioClips [6]);
				PlayerPrefs.SetInt ("Slide", 1);
			}

			float decelerate = Mathf.Min(decelerationRate * decelerateTime, 1f);
    ...
    private void SetPagePositions() {
        ...
        for (int i = 0; i < _pageCount; i++) {
			...
			child.localPosition = new Vector2 (child.localPosition.x, 50);
